...
Page::Page(LocationPtr location, uint32_t dest_id, const int id, const size_t size, const bool lazy, uintptr_t address)
    : location_(location), dest_id_(dest_id), page_id_(id), size_(size), lazy_(lazy), header_(reinterpret_cast<PageHeader *>(address)) {
    assert(address > 0);
}

Page::~Page() {
    release_mmap_buffer(address(), size_, lazy_);
}
...
PagePtr Page::load(LocationPtr location, uint32_t dest_id, int page_id, bool is_writing, bool lazy) {
    uint32_t page_size = location->journal_page_size;
    std::string path = get_page_path(location, dest_id, page_id);
    uintptr_t address = load_mmap_buffer(path, page_size, is_writing, lazy);
